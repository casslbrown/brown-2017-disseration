...
#path_input_bio <- "./data-unshared/derived/0-dto_b.rds"
path_input_biomarker <- "../HRS/data-unshared/derived/dto_b.rds" # product of 2-add-biomarker-data.R

path_output     <- "./data-unshared/derived/2-dto_b.rds"
path_output_wide <- "./data-unshared/derived/2-dto_wide_b.rds"
...
class(ds_bio$year)
ds_bio$year <- as.numeric(as.character(ds_bio$year))
class(ds_long$year)
ds_long$year <- as.numeric(as.character(ds_long$year))

#rename hhidpn to id for biomarker data
ds_bio <- ds_bio %>% dplyr::rename(id = hhidpn)

ds_list <- list()
ds_list[["data"]] <- ds_long
ds_list[["biomarker"]] <- ds_bio
...
ds <- merge_multiple_files(ds_list, by_columns = c("year","id","hhid"))

# check that crp means and sd's match published data.
aggregate(crp_adj~year, ds, mean)
aggregate(crp_adj~year, ds, sd)
...
variables_static <- c(
  ...
  ,"age_baseline"            # Age at baseline meaning 2004 in this case 
  ,"age_baseline_65"         # Age at baseline is age at the first wave when the partipant was 65 or older
  ,"health_conditions_baseline" # Health conditions at the first wave when the participant was 65 or older
  ,"healthcond_mean"         # Mean number of health conditions across waves when health condition data was available
) # static
...
# # excludes participants younger than 65 at first included wave (2004)
 ds <- dplyr::filter(ds, age_baseline > 64) 
 length(unique(ds$id))
...
mean(d_wide$age_baseline_65)
range(d_wide$age_baseline_65)
range(d_wide$intage_r_2006)

sum(is.na(d_wide$hhid))

# ---- save-to-disk ----------------------------------------
saveRDS(ds, path_output)
saveRDS(d_wide, path_output_wide)

# ---- save-mplus-data -------------------
write.table(d_wide, "./data-unshared/derived/wide-dataset-b.dat", row.names=F, col.names=F)
write(names(d_wide), "./data-unshared/derived/wide-variable-names-b.txt", sep=" ")

write.table(d_wide, "./data-unshared/derived/wide-dataset-baseline65.dat", row.names=F, col.names=F)
write(names(d_wide), "./data-unshared/derived/wide-variable-names-baseline65.txt", sep=" ")
